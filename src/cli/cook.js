...
function usageReport() {
  return {
    ...
    stdout: [
      ...
      '  claim <id> [--by <label>]  Atomically claim a task.',
      '  release <id> Release a task claim.',
      '  claims       List active task claims as JSON lines.',
      '  ls           List tasks.',
...
async function main() {
  ...
  if (sub === 'record') {
    ...
    try {
      const observedAgentId = councilRecord
