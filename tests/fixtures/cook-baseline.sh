...
Integration: feature branch → PR → auto-merge after CI, --merge; wait-for-land = PR merged.
...
main() {
  ...
  case "$sub" in
    ...
    verify)
      if [ "${1:-}" = "--task" ]; then exec node "$JEFF_ROOT/src/cli/cook.js" verify "$@"; fi
      cmd_verify "$@"
      ;;
    record)   exec node "$JEFF_ROOT/src/cli/cook.js" record "$@" ;;
