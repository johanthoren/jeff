...
5. **Git.** `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) drives the full-mode commit path. Capture/backlog may commit to trunk before implementation. For a simple task, implementation-pipeline changes remain in the worktree through the full gate, review, required audit, and done validation; terminal-with-removal then creates the task's sole green trunk commit. For a complex task, auto-commit at stage boundaries on a short-lived **local** branch `task/<id>-<slug>`, then squash-merge to trunk as one green commit on `done` and delete the branch (no remote branch, no PR; lessons go to project memory). Lite integration follows the operating profile. Classify by complecting, not difficulty; deployment ⇒ complex; default complex when unsure; decide at plan. Run `cook validate` before every commit. Never block on a dirty tree; interrupt the Chef rarely. **Terminal-with-removal:** a task reaching `done`/`abandoned` is **pruned** from the store: strip its id from live tasks' `deps`, `git rm -r` its dir, refresh BACKLOG, and commit the removal to trunk carrying the one-line outcome (`task <id> · done: <outcome (+ release tag)>` or `task <id> · abandoned: <why; superseded by …>`); for a simple task this commit also contains all implementation-pipeline changes, and for a complex task it is the squash-to-trunk commit. A done/abandoned dir never rests in the committed store (the archive is git history/tags + memory, not a `0NNN/` dir); `cook validate`'s `[prune]` enforces this (full mode only). The completion order is gate -> remove -> validate -> commit (the done-gate validates the present record, then the dir is removed before any commit), so a legitimately-completing task is never blocked.
