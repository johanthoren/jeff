...
**Verification protocol.** Code tasks keep the full test protocol in `skills/cook/SKILL.md`: stages run targeted tests, then Jeff runs the full suite exactly once after the last code-changing stage. Operation tasks do not manufacture tests or code review. Their plan declares canonical verification seams and `requiresApproval`. Pi gives the verifier fixed plan-bound Git/HTTPS reads and gives only the parent an exact approval UI; Claude Code and Codex fail closed when they cannot provide the required verification or approval boundary. The executor cannot self-attest a grant. Done retains the plan and requires nonempty execution evidence plus a fresh verifier, different from the executor, to report every planned postcondition in identical order and text with a true result and nonempty evidence; verify and required audit run concurrently, and the auditor differs from the executor.
