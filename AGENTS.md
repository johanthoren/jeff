...
5. **Git.** `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) drives the full-mode commit path. Capture/backlog may commit to trunk before implementation. A simple task keeps implementation-pipeline changes uncommitted only until the last code-changing stage is targeted-green, then creates a disposable local `task/<id>-<slug>` branch, validates, and makes a green checkpoint commit. Run `cook verify` on that clean checkpoint and record its HEAD as `tests.gate.hash`; a code kickback produces a fresh checkpoint and gate hash. A complex task retains stage-boundary commits on its local task branch. After review/audit and terminal-with-removal, commit the terminal preparation on the branch, squash it onto trunk, stage the complete terminal tree, and require `git diff --cached --quiet "$gate_hash" -- ':(exclude).jeff'` before the final validate and single trunk commit. This binds shipped content to the clean gate while allowing required `.jeff` cleanup. Delete the branch afterward. Lite retains its done ledger, reflects plan-store progress, and follows its operating profile. Classify by complecting, not difficulty; deployment ⇒ complex; default complex when unsure; decide at plan. Run `cook validate` before every commit. Never block on a dirty tree; interrupt the Chef rarely. A full-mode terminal task is pruned from the store; no terminal dir rests in committed state.
