...
When council selects the direct `refactor` route, this behavior-changing recovery contract overrides the ordinary behavior-preserving entry condition only for that dispatch. You are the episode's fresh builder, distinct from the prior builder, council members, and judges. The next gate and judgments remain the normal independent ones.
