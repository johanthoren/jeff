...
- Inspect the change for injection, broken authz/authn, SSRF, unsafe deserialization, path traversal, secret exposure, unsafe defaults, and risky new dependencies (prefer secure-by-default libraries over hand-rolled crypto/validation). The `security-auditor` skill, bundled at `skills/security-auditor/SKILL.md`, informs the workflow; keep the review bounded. If a bundled skill path this role names does not resolve, return `needs-work` for missing audit input instead of auditing without it.
