...
For a council-selected `confined-repair`, `causal-subgraph-reconstruction`, or `full-replan`, you are the single recovery builder. You must be fresh relative to the prior builder, recovery test author, council members, and judges. Preserve the task lineage and recovery evidence; a failed return exhausts the episode instead of opening another retry.
