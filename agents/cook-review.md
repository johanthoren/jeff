...
Inputs: the task spec (`task.md`), the plan, the diff (implementation + refactor), and the tests. Optional `context.md` is a facts-only map from plan; verify it against the code, do not trust it. Read the inputs. Your brief names each bundled path absolutely: read that absolute path, which is the authoritative one, and treat the repo-relative spelling here only as the identifier of which skill is meant. If such a path is missing from the brief or does not resolve, return `needs-work` for missing review input instead of judging without it.
