...
- **Mechanical council record.** After every contested blocker has a source-bound refute, write one strict council JSON file and run `cook record council <task-id> <file>`. The file is `{"stage":"council","council":{"convened":true,"stage":"review|audit","members":[...],"findings":[...],"verdict":"ship|block","outcome":"shipped"|null}}`. An initial block must record `outcome:null`. Members and findings use the canonical `convergence.council` shapes. The recorder persists the supplied votes and routes the resulting state; validation re-derives the majority and verdict. The recorder does not dispatch members, cast votes, or re-classify findings.
- **Termination: on BLOCK → one scoped fix+verify, else Chef.** Kick back to `implement` to resolve **exactly** the surviving findings. Record that scoped implementation, then archive the prior judgments and reopen only the affected slots while preserving their historical outcomes and identities. Dispatch fresh separated review and audit judgments at the new cycle. After those returns, establish the final content checkpoint and run fresh full verification plus `cook validate`. On PASS, record the same council aggregate again with only `outcome` changed to `scoped-fix-shipped`, then spin the demoted follow-ups. A failed reassessment or scoped implementation records `blocked-to-operator` and returns the task to the Chef. A direct initial `scoped-fix-shipped`, an unrecorded scoped implementation, or a missing fresh clean gate is rejected. **At most one implement cycle after a council**: this is the termination guarantee (INV-11).
