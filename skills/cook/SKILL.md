...
**Per-stage cap = 2 — run the verb, obey the word.** `review` and `audit` carry **independent** blocking-kickback counters in `convergence.stages.*.blockingKickbacks`; follow-ups never increment them. Do not reason the cap arithmetic by hand: for each **surviving** blocking kickback (call it once per survivor, after the refute pass has already demoted the refuted findings — never for a refuted finding), run `cook kickback <id> <stage>`. It increments the stage's counter (creating the `convergence` block as needed) and prints the decision word; **obey the printed word**: `kickback` → re-fire that earlier stage; `council` → **convene the council for that stage** (Dispatch → Council) instead of a 3rd kickback. The verb only counts and prints — it never convenes, never sets `council.verdict`/`members`; that stays your orchestration. Counters land in `convergence` so `cook validate` (INV-7) can check them.
