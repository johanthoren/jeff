...
1. **`cook prune <id>`.** It strips satisfied deps from every still-live (pending/in_progress/blocked) sibling, `git rm -r`s the dir (`.jeff/tasks/0NNN-<slug>/`), runs `cook validate` on the now-clean tree, and on success prints `git commit -m 'task <id> · done: <outcome (+ release tag)>'` (or `… · abandoned: <abandonReason>`) to stdout. On validate failure it surfaces `validation FAILED`, prints **no** commit line, and commits nothing — the staged rm + stripped deps are left uncommitted for you to inspect (`git restore` to undo).
