...
- **Explore:** ordinary engineering intent defaults to ad hoc work in the current context. Create no task or plan-store item and dispatch no specialist. Quick, reversible experimentation stays here unless the Chef explicitly asks to track it.
- **Remember:** preserve a durable finding without creating work. In full mode, write it under `.jeff/memory/`. Outside full mode, prefer a suitable existing Git-tracked memory, decisions, learnings, or handoff file. Preserve that file's purpose and format. If none exists, use local `.jeff/memory/`. Never use `AGENTS.md`, a README, or ordinary product documentation as a memory dump.
- **Record future work:** when the Chef asks to record an obligation, create or update a pending item and return to the current work. A full-mode task rests at `status: "pending"`, `stage: "capture"`; an external plan-store item remains pending and unadopted. Recording it does not start capture, adoption, planning, implementation, or specialist dispatch.
- **Start tracked work:** enter capture or the lite adoption pipeline only when the Chef separately asks to start the item or confirms a proposal to do so. Tracking consent and execution consent are distinct.
...
- **Termination: on BLOCK → one scoped fix+verify, else Chef.** Kick back to `implement` to resolve **exactly** the surviving findings. Record that scoped implementation, archive all prior review and audit judgments, and clear every required judgment slot. Dispatch fresh separated reviews and audit at the new cycle. Active-cycle judges must differ from the scoped implementer; historical judges may serve again. If a reassessment blocker survives refute, record `blocked-to-operator` immediately; it must not open another implementation cycle. After all fresh judgments pass, establish the final content checkpoint and run fresh full verification plus `cook validate`. The recorder requires that checkpoint still be the current HEAD with a clean non-`.jeff` tree. On PASS, record the same council aggregate again with only `outcome` changed to `scoped-fix-shipped`, then record the demoted follow-ups as pending. A failed reassessment or scoped implementation records `blocked-to-operator` and returns the task to the Chef. A direct initial `scoped-fix-shipped`, an unrecorded scoped implementation, or a missing fresh clean gate is rejected. **At most one implement cycle after a council**: this is the termination guarantee (INV-11).
...
- **Follow-up** = fail-safe edges, cosmetics, "could harden," degenerate-FS edges. → never blocks; record a pending follow-up item and its id (the parent ships regardless). In full mode the follow-up rests at pending/capture; outside full mode it remains pending and unadopted. It does not start without separate explicit intent or confirmation.

**Refute before you kick back.** Each blocking finding, before it becomes a kickback, gets one `cook-refute` dispatch: a fresh specialist, distinct id from the finder and the implementer, testing exactly that finding's reachability and severity honesty (several findings refute in parallel). `survives` → the kickback proceeds and the stage's counter increments. `refuted` → transcribe the demotion to follow-up with the refuter's recorded rationale and record the pending follow-up task; refuted findings never increment `blockingKickbacks`. You never re-classify in either direction: the refuter is the only voice that can demote a blocker, and only with recorded evidence.
