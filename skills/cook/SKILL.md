...
- **Termination: on BLOCK → one scoped fix+verify, else Chef.** Kick back to `implement` to resolve **exactly** the surviving findings. Record that scoped implement return separately, then run a fresh full-suite gate at its clean checkpoint. On PASS, record the same council aggregate again with only `outcome` changed to `scoped-fix-shipped`, then spin the demoted follow-ups. A direct initial `scoped-fix-shipped`, an unrecorded scoped implementation, or a missing fresh clean gate is rejected. On FAIL, record `blocked-to-operator` and hand to the Chef. **At most one implement cycle after a council**: this is the termination guarantee (INV-11).
