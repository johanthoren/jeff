...
4. **Integration is serialized at the main checkout, in completion order.**
   ...
   - If the ancestry check fails or an expected-old mismatch occurs, leave trunk unchanged and do not record the final passing return. Run `cook rebuild <id>`, which archives the gate and every review and audit judgment earned against the stale checkpoint, then rebuild the checkpoint from current trunk through the lane's existing normal recovery path. A rebuilt checkpoint therefore re-runs its own full-suite gate and re-dispatches review and required audit with fresh identities; a judgment from the discarded checkpoint can never satisfy its successor.
