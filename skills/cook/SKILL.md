...
- **Establish it with a local full run when the state is unknown**: session start, after any out-of-band change to the tree, or when the prior task did not finish green. Once established, **carry it forward**: each post-change green is the next task's baseline. Across sessions, **in full mode** `cook baseline check [<commit-ish>]` answers whether the current tree (`HEAD^{tree}`) is already a logged green+clean baseline (from a prior `cook verify`), so a known-good tree need not be re-run — including after a squash-merge of identical content (same tree, new commit). (**In lite mode the run log is empty**, so there is no logged baseline to check.)
...
- **In full mode**, the durable **tree-keyed run log** (`.jeff/test-runs.jsonl`, git-excluded) and the `cook verify` / `cook baseline check` helpers make carry-forward work **across** sessions: `cook verify` logs each gate verdict keyed by `HEAD^{tree}` + tree-dirty flag (with the commit recorded informationally), and `cook baseline check` reads that log to confirm a green+clean baseline at the current tree. **In lite mode nothing is logged** (`.jeff/` is the team's tree, not jeff's): a lite orchestrator binds each gate record to `git rev-parse HEAD` **directly**, never `tail`-ing `test-runs.jsonl` (which is empty).
