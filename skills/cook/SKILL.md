...
- **Name the path in the brief** so the specialist can re-read it (`cat .jeff/profile.md`) or follow `sources[*].path` into instruction and convention documents authoritative for the current host and repository (`AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING`, or equivalents, only when applicable) when they need more detail than the distillate.
