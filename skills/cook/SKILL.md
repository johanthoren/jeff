...
Brains are **assignment, not enforcement**. Each dispatched stage's brain (both `model` and `effort`) is pinned in `agents/cook-<stage>.md` frontmatter and is the single source of truth; Jeff does not rank, floor, or raise it. The settled values: `plan`/`review`/`audit` at `opus·xhigh`, `implement` at `opus·high`, `refactor` at `opus·xhigh` (zoom-out dedup/harmonization is judgment work),
`test` at `sonnet·medium` (a deliberately low-effort encoder so it does not wander, overfit, or invent its own test theory), and `refute` at `opus·xhigh` (it can overturn a judge's blocker). "Judge ≥ builder" holds by **construction** for the quality gate: review/audit (`opus·xhigh`) ≥ implement (`opus·high`), and `test` runs Sonnet because its output is fully fenced: it encodes the plan faithfully, exercising no judgment. Only `capture` runs on **you (Jeff)** at the top brain; it is not frontmatter-pinned.
...
**Review + audit dispatch in parallel.** They are independent read-only judgments of the same finished code, so once the last code-changing stage is green, dispatch `review` and `audit` (when the plan flagged a sensitive surface) **concurrently**, not serially, and collect both verdicts. Every other dispatched stage runs serially.
...
**Severity gate (from cycle 1).** Every `review`/`audit` finding is self-classified by the specialist (at its top brain) as **blocking** or **follow-up**. You only transcribe the label; you never re-classify.
