...
- In full mode, capture/backlog may commit to trunk before implementation, decoupled from any code branch. For a **simple** task, retain all implementation-pipeline code, tests, artifacts, and state changes in the worktree through the full gate, review, required audit, and done validation. Terminal-with-removal then commits those changes once to trunk. Intentionally red tests therefore never enter trunk history.
