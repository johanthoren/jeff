...
4. **Validate, commit the terminal preparation on the local task branch, then integrate as specified in Git.** The one final trunk commit carries the outcome message: `task <id> · done: <outcome (+ release tag)>` or `task <id> · abandoned: <why; superseded by …>`. It is the task's permanent, greppable trail (`git log --grep 'task .* · done'`).
...
4. **Integrate** the stage's changes according to Git: commit each boundary on a full-mode complex branch; for a full-mode simple task, retain changes only until the last code-changing stage is targeted-green, then create its local branch and green checkpoint; in lite, follow the operating profile. Then **advance** `stage`; on a kickback, set `stage` to the earlier stage with a recorded reason.
5. Repeat until the task reaches `done` (or blocks/abandons).
6. **Handle the terminal by mode.** In full mode, run terminal-with-removal and the bound squash-to-trunk sequence in Git and Validation. In lite, retain the done ledger, reflect terminal progress through the plan-store seam, and perform only the reversible integration or handoff allowed by the operating profile.
...
For each *dispatched* stage (`plan`, `test`, `implement`, `refactor`, `review`, `audit`, and `refute` when needed), dispatch a fresh subagent:
- **Claude Code:** use the native Agent/Task tool with `subagent_type: cook-<stage>`. Dispatch by that type and **never read its definition file**; its pinned brain (frontmatter) and stage contract (body) load automatically. Do not filesystem-search the plugin cache for it; resolve it by type.
- **Pi:** use `cook_dispatch` with `stage`, `brief`, and optional `taskDir`. It starts a fresh Pi role session from `agents/cook-<stage>.md` and returns `agent_id`, `stage`, `brain`, and transcript/verdict. Pi dispatch is model-agnostic: it inherits the orchestrator's current Pi model/provider and applies only the stage `effort:` as the requested Pi thinking level. The `model:` frontmatter remains for Claude Code compatibility and is ignored by Pi.
- **The brain source depends on the host.** Claude Code reads both `model:` and `effort:` from `agents/cook-<stage>.md`; Pi reads only `effort:` and never maps model aliases, ranks providers, or switches to another provider/model. Do **not** pass any `model` / `effort` argument or thinking-directive prose at dispatch. `task.json.brains.<stage>` is kept as an informational record of the plan-time intent; it no longer drives dispatch or validation.
...
- In full mode, capture/backlog may commit to trunk before implementation, decoupled from any code branch. For a **simple** task, retain implementation-pipeline changes in the worktree only until the last code-changing stage is targeted-green. Then create the short-lived local branch `task/<id>-<slug>`, run `cook validate`, and commit one green checkpoint. Run `cook verify` on that clean checkpoint and record `git rev-parse HEAD` as `tests.gate.hash`. Intentionally red tests and red full gates never enter trunk history. If a kickback changes code, make and verify a fresh green checkpoint and replace the recorded gate hash.
- For a full-mode **complex** task, auto-commit each stage's changes on its short-lived local task branch with `task <id> · <stage>: <summary>`. Stage commits accumulate as before. Run the full gate on its clean final code checkpoint.
- After review/audit and full-mode terminal-with-removal, run `cook validate` and commit the terminal preparation on the task branch. Switch to trunk, squash the branch, and stage the complete terminal tree. With `gate_hash` set to `tests.gate.hash`, require `git diff --cached --quiet "$gate_hash" -- ':(exclude).jeff'`; any difference means the shipped non-`.jeff` content was not gated and must not commit. Run the final `cook validate`, make the single trunk commit, then delete the local branch. The diff permits only `.jeff` to differ from the clean checkpoint for dependency stripping, BACKLOG refresh, and task removal.
- In lite mode, retain the terminal ledger, reflect progress through the plan-store seam, and follow the operating profile's reversible integration or handoff. Do not run full-mode pruning or impose either full-mode branch path. `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) selects only the full-mode paths above. Classify by complecting, not difficulty; deployment or other non-local side effects ⇒ complex; default complex when unsure; make or refine the call at plan.
- **The one trunk commit carries the outcome.** Its message is `task <id> · done: <what shipped (+ release tag if any)>` or `task <id> · abandoned: <why; superseded by …>` (e.g. `task 49 · done: optional per-AC test taxonomy; kill change-detector generator (-> 0.10.0)`; `task 12 · abandoned: cook bind; superseded by inference`). There is no separate full-mode done-record file: `git log --grep 'task .* · done'` is the greppable archive.
...
- **Jeff runs the full test suite exactly once per code checkpoint**, with the project's test command, **after the last code-changing stage**: `refactor` in the happy path, or the re-fix when a kickback reopened the code. In full mode the code checkpoint must be committed and clean before `cook verify`; its HEAD becomes `tests.gate.hash`. This run is the suite-wide gate.
...
The **`[prune]`** check is a **full-mode registry invariant**: a `done`/`abandoned` task dir must **not** rest in the store (the archive is git history/tags, not a resting dir). Like the other registry invariants (numeric id, INV-5 deps, duplicate-id), lite mode drops it: a lite Chef's external tracker owns the lifecycle and a lite run-ledger may legitimately retain a local `done` record. Because a present `done` record (which the done-gate `[gate]`/INV-4 validates) and an absent terminal dir (which `[prune]` requires) cannot both hold in one committed tree, full-mode completion follows a fixed **gate -> remove -> validate -> bind -> commit** ordering:
1. The task earns `done` (record `status=done` + gate/review/audit; dir still present). Keep the clean gate hash unchanged after review/audit.
...
4. **`cook validate` #2** is now clean. Commit this terminal preparation on the local task branch.
5. Switch to trunk, squash the branch, and stage the complete terminal tree. Require `git diff --cached --quiet "$gate_hash" -- ':(exclude).jeff'`, where `gate_hash` is the recorded clean `tests.gate.hash`.
6. Run the final `cook validate`, commit once to trunk with the one-line outcome, and delete the local branch. This committed tree never rests a terminal dir, and its shipped content matches the clean gate.

Run step 3 (the removal) and each later commit as **separate tool calls**, never batched in one shell command. Remove first, let `cook validate` #2 confirm the pruned tree, then commit the terminal preparation in a later call.
