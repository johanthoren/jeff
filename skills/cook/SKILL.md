...
4. **Integrate** the stage's changes according to Git: commit the boundary on a full-mode complex branch, retain them in the worktree for a full-mode simple task, or follow the lite operating profile. Then **advance** `stage`; on a kickback, set `stage` to the earlier stage with a recorded reason.
...
- In full mode, capture/backlog may commit to trunk before implementation, decoupled from any code branch. For a **simple** task, retain all implementation-pipeline code, tests, artifacts, and state changes in the worktree through the full gate, review, required audit, and done validation. Terminal-with-removal then creates the task's sole green trunk commit. Intentionally red tests therefore never enter trunk history.
- For a full-mode **complex** task, auto-commit each stage's changes on a short-lived **local** branch `task/<id>-<slug>` with `task <id> · <stage>: <summary>`. Stage commits accumulate there, then squash-merge to trunk as one clean green commit on `done` and delete the branch (no remote branch, no PR; capture any lasting lesson in project memory, never a surviving branch).
- In lite mode, integration follows the operating profile; do not impose either full-mode path. `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) selects only the full-mode paths above. Classify by complecting, not difficulty; deployment or other non-local side effects ⇒ complex; default complex when unsure; make or refine the call at plan. Every trunk commit must be green.
- **The terminal commit removes the task dir and carries the one-line outcome.** When a full-mode task reaches `done`/`abandoned`, its dir is `git rm`'d and the removal committed to the trunk (the terminal-with-removal sequence). The message is the per-task permanent trail, so it carries a one-line outcome: `task <id> · done: <what shipped (+ release tag if any)>` or `task <id> · abandoned: <why; superseded by …>` (e.g. `task 49 · done: optional per-AC test taxonomy; kill change-detector generator (-> 0.10.0)`; `task 12 · abandoned: cook bind; superseded by inference`). For a **simple** task this commit also contains all implementation-pipeline changes; for a **complex** task it is the squash-to-trunk green commit. There is no separate per-task done-record file: `git log --grep 'task .* · done'` is the greppable, zero-bloat archive.
