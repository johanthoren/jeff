...
**When a task reaches a terminal state (`done` or `abandoned`), prune it from the store (terminal-with-removal).** A done/abandoned task dir must **not** rest in the committed full-mode store; the archive is git history and tags, not a resting `0NNN/` dir. The mechanical half is one verb: **`cook prune <id>`** (full mode only) strips the finishing id from every still-live sibling's `deps`, `git rm -r`s the task dir, re-validates the store, and on green **prints** the exact commit command for you to run. The sequence (same for `done` and `abandoned`; the only difference is the printed message, and that an abandoned task records `abandonReason` which the verb reads into the printed line):
1. **`cook prune <id>`.** It strips satisfied deps from every still-live (pending/in_progress/blocked) sibling, `git rm -r`s the dir (`.jeff/tasks/0NNN-<slug>/`), runs `cook validate` on the now-clean tree, and on success prints `git commit -m 'task <id> · done: <outcome (+ release tag)>'` (or `… · abandoned: <abandonReason>`) to stdout. On validate failure it surfaces `validation FAILED`, prints **no** commit line, and commits nothing; the staged rm + stripped deps are left uncommitted for you to inspect (`git restore --staged --worktree .` / `git reset --hard` to undo).
2. **Refresh BACKLOG.md.** The verb does **not** touch BACKLOG; this stays your step. **Remove** the finished task from BACKLOG entirely (NOW/NEXT/TODO) and write **no** done-record or release narrative: the archive is git tags/history and memory, not BACKLOG. Optionally promote NEXT→NOW for the next task, and file newly-spun follow-up ids into TODO (or NEXT if imminent).
3. **Run the printed commit** (finalizing the outcome text) as a **separate tool call**, never `cook prune … && git commit` in one shell command (see Validation for why the hook still trips). This single commit is the task's terminal artifact and its permanent, greppable trail (`git log --grep 'task .* · done'`). For a **complex** task, the squash-to-trunk green commit **is** this removal commit.
...
6. **On a terminal state (`done`/`abandoned`), prune the task** with **`cook prune <id>`** (the terminal-with-removal sequence above and the completion ordering in Validation): the done-gate validates the present record, then the verb strips deps + removes the dir + re-validates and prints the removal commit, which you run separately. A done/abandoned task never rests in the committed store; `cook validate`'s `[prune]` check enforces this.
...
- **The terminal commit removes the task dir and carries the one-line outcome.** When a task reaches `done`/`abandoned`, `cook prune <id>` `git rm`s its dir and prints the removal commit for the trunk (the terminal-with-removal sequence). The message is the per-task permanent trail, so it carries a one-line outcome: `task <id> · done: <what shipped (+ release tag if any)>` or `task <id> · abandoned: <why; superseded by …>` (e.g. `task 49 · done: optional per-AC test taxonomy; kill change-detector generator (-> 0.10.0)`; `task 12 · abandoned: cook bind; superseded by inference`). The verb prints an outcome placeholder (and the recorded `abandonReason` for an abandoned task); you finalize the outcome text before running the commit. For a **simple** task this is one direct-to-trunk removal commit; for a **complex** task the squash-to-trunk green commit IS the removal commit. There is no separate per-task done-record file: `git log --grep 'task .* · done'` is the greppable, zero-bloat archive.
...
3. **`cook prune <id>`** (terminal-with-removal): it strips the task's id from live siblings' `deps`, `git rm -r`s the dir, runs **`cook validate` #2** internally (now clean: no resting terminal dir, no dangling dep), and prints the removal commit. Refresh BACKLOG as a distinct step (the verb does not touch it).
4. **Run the printed commit** to trunk with the one-line outcome. This committed tree never rests a terminal dir.

Run `cook prune <id>` (step 3) and the printed commit (step 4) as **separate tool calls**, never batched in one shell command. The pre-commit validate-hook inspects the on-disk tree *before* the command runs, so a combined `cook prune … && git commit` trips `[prune]` on the still-present `done` dir (the prune's `git rm` has not touched disk yet when the hook reads the tree) and blocks the commit. Prune first, let its internal `cook validate` #2 confirm clean, then commit in a second call.
