...
3. **Record** the specialist's strict JSON return through `cook record <stage> <id> <file>` (agent id, verdicts, findings, and evidence). Pi dispatch with a task id calls the same recording core directly. Append narrative notes to `notes.md`; keep returned child-session provider/model/effort as execution evidence.
