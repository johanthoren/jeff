...
- **Explore:** the normal host agent handles ordinary engineering intent in the current context under the applicable user, host, and repository instructions. Create no task or plan-store item and dispatch no specialist. Addressing Jeff or the Chef, using engineering verbs, or working in an active project does not change this route. **Assess first (read-only / non-mutating).** Before the **first durable write**, classify and route by the gate below; do not edit, scaffold a task, or cut a version ahead of it.
...
**Classify, then fork.** Two axes: consequence (what breaks if it is wrong) and expected lifetime (how long the artifact lives). State the class and a one-line reason at the routing moment. Disposable work (a throwaway experiment, a comparison, a local evidence collector, a one-off helper) takes route **A** directly, with no stop and no A/B/C question, even when a structural signal below such as multi-file or cross-cutting fires, since those signals are proxies for consequence. Call that result ad-hoc: it is not pipeline-verified, and it is deleted or explicitly kept local once the result exists. A risk floor always overrides a lighter classification and restores the ordinary pause: production behavior, user data, data migration, security boundaries, accessibility basics, irreversible or shared state, releases, and durable build or deploy infrastructure. Promote work to the full path as its expected lifetime or blast radius grows.
...
Named start (`cook <id>` / `cook on <ref>`) claims a ready task even when active claims already equal `maxParallelTasks`. After claiming, that start always uses a linked worktree and task branch. `cook all` refuses to open a new autonomous claim once the cap is full.

Lite landing is a feature branch, push, and open PR. Lite landing does not use trunk compare-and-swap. Multiple PRs may be open at once. When the final required passing return would cause the terminal transition, hold only that final passing return unrecorded (same hold as full-mode step 4) until wait-for-land succeeds. After the done-gate Jeff opens the PR and enables auto-merge after CI unless Integration: or a live request forbids landing, then wait-for-land, restore-if-needed, record that held return and done, then post-land checkout.
...
5. Repeat until the task reaches `done` (or blocks/abandons). Reaching `done` waits for the step 6 hold: do not record the terminal passing return at step 3.
6. **Handle the terminal by mode.** When the final required passing return would cause the terminal transition, hold only that return unrecorded. In full mode, after landing, run terminal-with-removal and satisfy the Git and Validation outcomes. In lite, retain the ledger, reflect terminal progress through the plan-store seam, open the PR and enable auto-merge after CI unless the operating profile or a live request forbids landing; wait-for-land, restore-if-needed, record that held return and done, then post-land checkout.
...
**Bundled skill paths.** A dispatched station has no skill loader. Read the stage's `agents/cook-<stage>.md` for the Jeff-owned mandatory paths it names (`skills/testing/SKILL.md`), resolve each per §Resolving payload paths, and name those absolute paths in the brief. The matching language skill (`rust`/`swift`/`clojure`) is optional: resolve host, then repository, then another installed package, and name that absolute path only when it exists. Never invent a Jeff-local `skills/<lang>/SKILL.md`. Absence of the language skill is not a stop when the mandatory floor is present. Fail closed only when the brief claims a required path and that exact path does not resolve. Pi builds the brief path list through `resolveDispatchSkills` in `src/core/skill-dispatch.js`. Claude Code, Codex, Cursor, and Grok Build follow this same order; they do not get a second host-specific recipe.
...
For each dispatched stage (`plan`, `implement`, `refactor`, `execute`, `review`, `verify`, `audit`, `refute`, each `council` inquiry, and `council-synthesis` when needed), dispatch a fresh subagent:
...
- **Cursor:** use the native subagent / Task tool over the shared `agents/cook-*.md` definitions and record the host-observed child id separately; the specialist JSON omits `agent_id`. Grok Bot loads Jeff through the shipped Cursor plugin; native dispatch on the Bot computer is Grok Build plus cook.
- **Grok Build:** use the Claude Code-compatible plugin agent definitions through Grok Build's native subagent facility and record the host-observed child/task id separately; the specialist JSON omits it. Grok Build discovers Claude Code plugins, agents, skills, hooks, and marketplaces directly, so Jeff adds no parallel Grok-only method or manifest.
- **Pi:** use `cook_dispatch` with `stage`, `brief`, and `taskId` when recording. Council inquiry and synthesis sessions are the exception: dispatch them without `taskId`, read-only. Pi returns each validated inquiry or synthesis with the host-observed child id. Jeff assembles and records the aggregate only after binding all three member ids and the synthesizer id. Execute receives its ordinary editing tools; verify and audit receive read tools plus commands; review, refute, council, and council-synthesis receive read tools only.
...
The pipeline stops for the capture lock and for an operation's exact shared-mutation approval. The executor requests that approval and stops; Jeff presents the request, records the operator's exact grant through `cook approve <id> <operator>`, and only then re-fires execute. Otherwise the pipeline runs autonomously. A specialist with a genuine unresolved fork returns an escalation to Jeff; an operation plan escalation is recorded without mutation and stays at `plan` until Jeff grounds the question for the Chef and re-dispatches plan with the answer. Jeff does not ground plumbing as a Chef-facing escalation.

### Council (task-wide, triggered when a stage hits the cap)

When any code `review`/`audit` or operation `verify`/`audit` source reaches its cap, wait for every required active judgment and source-bound surviving refute, suppress ordinary kickbacks from that active union, then convene one task-wide council over the exact blocker union.

- Dispatch exactly three fresh, mutually distinct, read-only inquiries with decorrelated integrity, security, and pragmatist lenses. Each return omits `agent_id`; Jeff binds the host-observed child id. Members remain separate from builders and judges. At least one asks exactly: "Are these independent defects, or evidence that this part of the design should be reconstructed?"
- Each inquiry records `question`, `problemRestatement`, `causalHypotheses`, materially different `solutionStrategies`, one source-bound `findingVotes` entry per active finding, and `decisiveEvidence`. Semantic canonicalization ignores object-key order, whitespace, case, and punctuation when proving that the three inquiry packets are epistemically distinct.
- Jeff deterministically derives every finding tally, survival flag, survivor id, and ship/block verdict from all three inquiries. A finding survives iff at least two members mark it blocking. Demoted findings retain an existing follow-up task id or `"ledger"`; survivors retain `followupTaskId: null`.
- After the inquiries, dispatch a fourth fresh, independent synthesis specialist in the read-only `council-synthesis` role. That station owns `problemRestatement`, `survivingBlockers`, `causalHypotheses`, materially different `solutionStrategies`, `rejectedAlternatives`, `selectedStrategy`, and `decisiveEvidence`. Jeff only derives the vote result, binds the three host-observed member ids and host-observed synthesizer id, and transcribes the validated aggregate.
- A blocking canonical code council opens exactly one bounded same-task recovery episode. Its selected route is one of `confined-repair`, `test-contract-repair`, `refactor`, `causal-subgraph-reconstruction`, `full-replan`, or `operator-escalation`. Confined repair dispatches implement and never enters refactor; direct refactor requires a behavior-changing `refactored` return; causal reconstruction and full replan dispatch a fresh plan/test author followed by a distinct implementer; operator escalation blocks immediately.
- `test-contract-repair` is test-only recovery: a fresh plan/test author may add proof obligations and then route directly to the gate-facing review state without manufacturing a production implementation. Every production route records one fresh recovery builder. Recovery preserves the original complexity and reviewer floor and cannot lower an already-required audit.
- Jeff runs the normal clean green full-suite gate at the current recovered checkpoint before any fresh judgment. The same review count, audit floor, source-bound refutes, and deterministic done-gate apply. The first surviving post-recovery blocker exhausts the episode and sets `blocked-to-operator`; no second council, retry, successor task, or adoption can reset that lineage.
- The original plan, original test-author, original builder, original implementation, and original checkpoint remain durable with the original goal, task id, judgments, findings, refutes, votes, synthesis, and recovery evidence. Historical persisted council ledgers may omit inquiry, synthesis, synthesizer identity, and recovery only at rest.
- Operation synthesis selects only the operation-specific `scoped-execute` route or `operator-escalation`. `scoped-execute` enters the existing execute stage and remains recorded; a code-only route is rejected rather than ignored.
...
- Before every start or resume (bare cook, named start, Record+start, a cook all lane), read `cook claims` and in-flight tasks fresh from disk; never trust session context. The same check applies whether or not this session is running cook all. Claim the task being started before advancing it. After claiming, always use a linked worktree and task branch. Empty occupancy is not permission to take main.
- Repository and host context choose branch, checkpoint materialization, and integration mechanics. Routine reversible Git work is autonomous; interrupt only for unattributable changes, unresolvable conflicts, or force-push or history rewrite.
...
Hold every specialist to jeff's **bundled first-party** skills as a portable floor: `code-standards` (the baseline for all code), plus `testing` for the plan stage and `security-auditor` for audits. The matching language skill (`rust`/`swift`/`clojure`) is optional and comes from the host, repository, or another installed package using the §Dispatch order; do not duplicate those rules inside Jeff. Absence of a language skill does not invalidate a station that still has this mandatory floor and applicable repository instructions. Applicable user, host, repository, and language instructions may tighten or specialize this floor, never weaken it (language skills override per-language). Do **not** lean on third-party skills or built-in review/refactor tools (`/code-review`, `/simplify`) to drive behavior: jeff controls the bar. No AI or assistant attribution in commits.
