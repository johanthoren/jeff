...
- **Explore:** the normal host agent handles ordinary engineering intent in the current context under the user's and repository's ordinary instructions. It may inspect, edit, and run targeted checks as requested. Create no task or plan-store item and dispatch no specialist. Addressing Jeff or the Chef, using engineering verbs, or working in an active project does not change this route.
