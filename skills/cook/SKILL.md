...
**Capture interview.** `skills/cook/reference/capture-interview.md` owns the Chef-facing interview (one `ask` at a time, design tree, recommended answer), the durable tree in `task.md`, and the project `CONTEXT.md` / `docs/adr/` rules. Capture writes those project files before lanes start. Later stages never do.
