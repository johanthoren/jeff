...
For each dispatched stage (`plan`, `implement`, `refactor`, `execute`, `review`, `verify`, `audit`, `refute`, and each `council` inquiry when needed), dispatch a fresh subagent:
...
- **Pi:** use `cook_dispatch` with `stage`, `brief`, and `taskId` when recording. Council is the exception: dispatch each inquiry without `taskId`, read-only, then assemble and record the aggregate with the three host-observed ids. Execute receives its ordinary editing tools; verify and audit receive read tools plus commands; review, refute, and council receive read tools only. Pi projects an execute `approvalRequired` string exactly to the parent.
...
- Each inquiry records `question`, `problemRestatement`, `causalHypotheses`, materially different `solutionStrategies`, one source-bound `findingVotes` entry per active finding, and `decisiveEvidence`. The three inquiry packets must not be identical. Only after all three return does Jeff synthesize the problem restatement, surviving blockers, causal hypotheses, materially different strategies, rejected alternatives, selected strategy, and decisive evidence.
- Jeff deterministically derives each tally from the three independent votes. A finding survives iff at least two members mark it blocking; `survivingBlockers` lists exactly those finding ids, and the council blocks iff any survive. Demoted findings retain an existing follow-up task id or `"ledger"`; survivors retain `followupTaskId: null`.
- A blocking canonical code council opens exactly one bounded same-task recovery episode. Its selected route is one of `confined-repair`, `test-contract-repair`, `refactor`, `causal-subgraph-reconstruction`, `full-replan`, or `operator-escalation`. Confined repair dispatches implement; refactor dispatches a direct recovery refactorer; causal reconstruction and full replan dispatch a fresh plan/test author followed by a distinct implementer; operator escalation blocks immediately.
- `test-contract-repair` is test-only recovery: a fresh plan/test author may add proof obligations and then route directly to the gate-facing review state without manufacturing a production implementation. Every production route records one fresh recovery builder. Recovery test author, builder, council members, and fresh judges are distinct, and recovery cannot lower an already-required audit.
- Before fresh judgments Jeff runs the normal clean full-suite gate for the recovered checkpoint. The same review count, audit floor, source-bound refutes, and deterministic done-gate apply. The first surviving post-recovery blocker exhausts the episode and sets `blocked-to-operator`; no second council, retry, successor task, or adoption can reset that lineage.
- The original goal, task id, prior checkpoints, judgments, findings, refutes, votes, synthesis, and recovery evidence remain durable. Historical council ledgers that omit inquiry, synthesis, and recovery retain their existing scoped-fix semantics. Operation recovery also retains its existing scoped execute semantics rather than entering code-only routes.
