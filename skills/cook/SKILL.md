...
- **Cursor:** use the native subagent / Task tool over the shared `agents/cook-*.md` definitions and record the host-observed child id separately; the specialist JSON omits `agent_id`.
...
- **Grok Bot:** loads Jeff through the shipped Cursor plugin; native dispatch on the Bot computer is Grok Build plus cook.
