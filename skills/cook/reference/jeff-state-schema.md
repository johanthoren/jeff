...
- Code `plan.refactorOpportunity` carries a nonempty named behavior-preserving opportunity or `null`; historical code plans may omit it. A completed operation plan requires `runbook`, `preconditions`, `recoveryBoundary`, `approvalBoundary`, boolean `requiresApproval`, `postconditions`, and canonical `verificationSeams`, and omits code test/refactor fields. An unresolved operation fork instead persists only `result:"escalation"`, nonempty `slices`, and nonnull `{fork, options}` while remaining at `plan`; the answered plan replaces it.
...
- `audit`: `required` set by `plan`; `verdict` ∈ `pass | needs-work | na`. A required audit cannot record `na`.
- `execution`: operation execute result plus bound executor identity, nonempty action strings and nonempty command/output evidence, nullable `approvalRequired`, and optional parent-recorded `approval`. `approval-required` names the exact mutation and remains at `execute`. The executor return cannot contain a grant. On Pi, parent-only `cook_approve` shows the pending bytes in the parent UI, collects operator identity, and atomically records only an unchanged exact request. Claude Code and Codex fail closed at this boundary until they have an equivalent parent-authenticated channel.
- `approvals`: optional append-only operation approval history. Each closed record is `{ mutation, grantedBy, grantedAt }`, created only by the parent approval transition from the active pending mutation. A `requiresApproval:true` plan cannot execute directly, and done requires `execution.approval` to exactly match a retained entry.
- `verification`: operation verdict plus bound verifier identity, deterministic `{ postcondition, ok, evidence }` results, findings, and nonempty command/output evidence. Pi provides read-only file tools plus fixed `verify_query` requests that exactly match canonical plan seams. Claude Code fails closed before dispatch for unsupported Git or external-state seams; Codex fails closed because its generic child cannot be mechanically narrowed. Done retains the plan and requires exactly one row per `plan.postconditions` item in identical order and text, with every row true and evidenced, and verifier different from executor. Follow-ups and refute- or exact council-demoted findings remain durable without blocking completion.
