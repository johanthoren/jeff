...
- `complexity`: `"simple" | "complex"` (absent ⇒ `"complex"`): drives the full-mode commit path. A simple task retains implementation-pipeline changes for its sole terminal trunk commit; a complex task uses stage commits on a local branch before the terminal squash. Set or refine it at plan by whether the change complects (braids concerns, couples previously-separate things, crosses subsystem boundaries, or has non-local side effects); classify by complecting, not difficulty; deployment or other non-local side effects ⇒ `"complex"`; default `"complex"` when unsure.
- `branch`: `task/<NNNN>-<slug>` when `complex`, else `null` (the simple task lands once on trunk at terminal-with-removal).
...
- **Full mode** (the default): the committed task dirs, validated by the full invariant set below. Jeff runs `cook validate` before every commit; complex tasks commit at stage boundaries on their branch, while simple implementation-pipeline changes wait for the sole terminal trunk commit. CI runs `make validate` on push. No git pre-commit hook is installed in any mode.
...
- **full / absent.** Empty `tasks/` (no task dirs) ⇒ "nothing to validate", exit 0; otherwise runs the **full** invariant set over the on-disk task dirs: the schema/done-gate quality invariants (INV-1, INV-2, the `plan-sep` separation, INV-4; INV-3 was removed, brain tiering is now pinned assignment, not a validator floor), the convergence block (INV-7..11), **and** the registry invariants: numeric-`id` requirement, `deps` reference existing tasks + no cycles (INV-5), duplicate-id, and `[prune]`.
- **lite: quality subset only.** Runs INV-1 (test author ≠ implementer), INV-2 (implementer ≠ reviewer), `plan-sep` (plan agent ≠ implementer), INV-4 (done-gate), and the INV-7..11 convergence block over each run-ledger `task.json`. **Drops** the registry invariants: a string `id` (an external tracker ref, e.g. `"JIRA-42"`) is accepted, INV-5 (dep DAG), duplicate-id, and `[prune]` are **skipped** (a lite run-ledger legitimately retains a local `done` record). (INV-3, the old `review/audit brain ≥ implement` floor, was removed; brain tiering is now pinned assignment in `agents/cook-*.md`, not a validator invariant.)
...
- **INV-1**: `tests.authored_by_agent_id ≠ agents.implementer_agent_id` (the test *encoder* is not the implementer).
- **plan-sep**: `agents.plan_agent_id ≠ agents.implementer_agent_id` (the test *designer* is not the implementer). With `plan` dispatched and designing the test contract (behaviors + seams), the contract is shaped by two agents, `plan` (designer) and `test` (encoder), and the implementer must be distinct from **both**. Fires only when both ids are recorded and equal; absent / `null` ⇒ skipped, so pre-field task.jsons stay valid. Distinct marker `[plan-sep]`, not the retired `inv3`.
- **INV-2**: `agents.implementer_agent_id ≠ agents.reviewer_agent_id` (the reviewer did not write the code).
