...
- Operation approval requests and grants are append-only provenance. A pending request must match the planned boundary exactly. The executor cannot grant its own request, and completed approval-gated execution binds the active request and retained grant before verification. New recorder grants persist `requestId`. Omitted `requestId` remains the historical compatibility rule via the old latest-request checks, including two-request re-stop ledgers.
...
- A live canonical council records three distinct, host-observed members and three semantically distinct independent inquiry packets before synthesis. Object-key order, whitespace, case, and punctuation do not manufacture distinctness. At least one inquiry carries the exact defects-versus-reconstruction question. Every inquiry votes every active source-bound finding exactly once; tallies, survival flags, survivor ids, and verdict are derived from those votes for both ship and block.
- A fourth fresh, read-only synthesis specialist owns the problem restatement and selected recovery strategy. The persisted aggregate binds all three host-observed member ids and the host-observed synthesizer id. Jeff only derives and transcribes the validated result.
- A blocking canonical code council may add exactly one recovery episode to the same task. The recovery route is one of confined repair, test-contract repair, refactor, causal-subgraph reconstruction, full replan, or operator escalation, and must equal the synthesis selection. Test-contract repair moves from fresh plan/test authorship directly to the gate-facing review state; reconstruction and replan continue to implement; confined repair never enters refactor; direct refactor requires a behavior-changing recorded refactorer.
- Recovery retains the original plan, original test-author, original builder, original implementation, and prior checkpoint. Complexity, required reviewer count, and audit requirements are monotonic floors. Every production route binds its fresh builder to the recorded implementation or refactor.
- A clean green current-checkpoint gate must pass before fresh judgments. One surviving blocker exhausts the episode to the operator. Historical persisted councils may omit inquiry, synthesis, synthesizer identity, and recovery and retain their prior scoped-fix meaning.
- Operation synthesis accepts only the operation-specific `scoped-execute` route or operator escalation. `scoped-execute` reuses the existing execute stage; code-only routes are invalid rather than ignored.
