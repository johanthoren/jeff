...
- `judgmentHistory`: append-only archived judgment cycles. A code row records its time, exact `review`, `review2`, and `audit` outcomes, plus their bound identities. An operation row additionally records its zero-based `cycle` and exact verifier/auditor outcomes. Scoped repair appends the whole current row once; it never rewrites an earlier row. Attached and retained refutes remain distinct from their active or archived source judge.
- `kickbacks`: `[{ from, to, reason, at, findings? }]`. Ordinary code judgment kickbacks may add `findings: [{ source, file, line, what, kickTo }]`; `source` is `review | review2 | audit`, `file` and `what` are nonempty strings, `line` is an integer at least 1, and `kickTo` is `implement | refactor`. Absence and an empty array remain valid historical forms but cannot authorize scoped repair. Council-block kickbacks omit `findings` and retain their existing shape. Ordinary operation sources are `execute | verify | audit` and destinations are `capture | plan | execute`; a council-scoped execute kickback to capture or plan terminates as `blocked-to-operator`. Code keeps its existing graph, including historical `verify` source compatibility.
...
At the `implement` or `refactor` record following ordinary code judgment kickbacks, repair is scoped only when every applicable kickback in the active round has nonempty typed findings, every finding targets `implement` or `refactor`, the repair records a nonempty `files` subset of the finding file union, and no council is pending or convened. Category omission retains its historical code meaning. Every other path uses the existing full judgment reset.

A scoped repair appends the whole current code judgment entry to `judgmentHistory` once. A raising `review` source clears `review`, `review2`, and both reviewer identities. A raising `audit` source clears only `audit` and its identity. When both raise in one round, all judgment slots clear. Independently passing sibling outcomes and identities remain byte-for-byte equal to the archived row; convergence counters do not change. Settlement selects the now-vacant source. Every implement or refactor repair clears `tests.gate` and sets `tests.green` to false, so the full-suite gate remains the mandatory checkpoint anchor.

- **INV-12 (targeted repair retention):** code only. It is skipped entirely when `judgmentHistory` is absent or no judgment kickback carries nonempty `findings`; this leaves every historical ledger untouched. When live state presents a retained judgment, including an otherwise archived-equal outcome with a mismatched identity, the latest typed judgment kickback and every same-round judgment kickback must carry aligned findings with `implement | refactor` destinations. The latest history row must postdate that kickback round. A successful post-kickback implement or refactor record must list nonempty repair files confined to the union of finding files. Every populated sibling source that the round did not raise must retain a `pass` outcome, its exact archived identity in both identity owners, and an outcome deep-equal to the latest archived counterpart. Missing, stale, non-passing, source-mismatched, identity-mismatched, outcome-mismatched, or file-unconfined proof violates `[inv12]`.
