...
- `cook reverify <id>` applies only to an in-progress operation with completed execution and a current `needs-work` verification whose blocking findings do not require execute recovery. It appends the whole superseded judgment to `judgmentHistory`, clears only the live verification slot, leaves execution unchanged, and returns to `verify`; the next verifier must be fresh and distinct from both the executor and every archived verifier.
