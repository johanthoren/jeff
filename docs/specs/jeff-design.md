...
6. **Lean method, borrowed craft.** The craft (capture, TDD, review) is native to frontier models; we supply framing + conventions and hold work to jeff's *portable bundled first-party standards floor* (which applicable user, host, repository, and language instructions may tighten or specialize, never weaken). No dependency on method-imposing third-party packs: the floor is jeff's own bundled skill.
...
Specialists are held to jeff's portable **bundled first-party** `code-standards`/`testing`/`security-auditor` floor as the quality bar; applicable user, host, repository, and language instructions may tighten or specialize it, never weaken it. This does **not** depend on a third-party `code-standards` skill. jeff owns the method, state, conventions, and file locations. We write only:
