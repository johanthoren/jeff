...
8. **Commit green checkpoints when simple; retain stage commits when complex.** Jeff owns the working tree during a run. In full mode, a simple task keeps stage work uncommitted only until its last code-changing stage is targeted-green, then creates a disposable local task branch and one green checkpoint commit. `cook verify` runs on that clean checkpoint. Complex tasks retain stage-boundary commits on their local branch. The workflow **never halts to ask the Chef to stash or clean**; there is no dirty-tree gate or waiver dance.
9. **Be smart about git; make judgment calls; interrupt rarely.** Routine commit, branch, and squash work is autonomous inside a run (this is the intended override of any default "confirm before commit"). Jeff interrupts the Chef only when genuinely necessary: surprising unrelated changes it cannot attribute, an unresolvable conflict, or anything requiring force-push or history rewrite. `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) drives the full-mode commit path: **simple** tasks (do not complect) produce only green checkpoint commits on a disposable branch; **complex** tasks (complect: braid concerns, couple previously-separate things, cross subsystem boundaries, or carry non-local side effects such as a deployment) accumulate stage commits on that branch. Both paths record the clean gate HEAD, commit terminal preparation after removal, and squash onto trunk. Before the one trunk commit, the complete terminal tree is staged and `git diff --cached --quiet "$gate_hash" -- ':(exclude).jeff'` binds shipped content to the clean checkpoint while allowing `.jeff` cleanup. Delete the branch afterward. Classify by complecting, not difficulty; deployment or other non-local side effects ⇒ complex; default complex when unsure; make the call at plan. Backlog/capture commits may land on trunk before implementation. Lite retains done ledgers and follows profile-driven integration. Net: trunk history gets one green terminal commit per completed full-mode task; complex tasks also have granular stage history during the task's life.
...
**Branch checkpoints.** A full-mode complex task commits every stage transition on its local branch, preserving discrete stage diffs before the terminal squash. A full-mode simple task leaves red and intermediate stage work uncommitted, creates the same kind of local branch after the last code-changing stage is targeted-green, then validates and commits one green checkpoint. `cook verify` runs with that checkpoint clean and records its HEAD as the gate hash. A code kickback creates a fresh checkpoint and gate hash. Lite follows its operating profile. Jeff runs `cook validate` before each commit; CI runs it on push.
...
Jeff runs `cook validate` before every commit (CI runs it on push). On complex branches, structural invariants (5, 6) bind at each stage boundary; on simple branches, they bind at each green checkpoint. The separation/completion invariants (1, 2, 4) bind as their fields populate, with the full done-gate (invariant 4) enforced only when `status = done`. Before the final trunk commit, the staged non-`.jeff` tree must match the clean gate checkpoint.
