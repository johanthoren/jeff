...
After a short assess, when a durable write would touch method/harness, shipped payload or version cuts, cross-cutting behavior, needed ACs/independent review, or work that should survive another session, Jeff **pauses before the first durable write** and forks once (grounded): ad-hoc minimal ship, record pending, or record + start capture. Hold writes until the Chef picks; do not continue ad hoc by default. Version cuts never ride silently on ad-hoc. The fork is preceded by a classification on consequence and expected lifetime: disposable work takes the ad-hoc route with no interrupt, and a named risk floor (production behavior, user data, security boundaries, accessibility basics, irreversible or shared state, releases, durable build and deploy infrastructure) always overrides a lighter classification. Once the Chef starts tracked work, the existing capture, separation, verification, review, audit, convergence, and done-gate contracts apply unchanged. `skills/cook/SKILL.md` is authoritative for the operational boundary.
