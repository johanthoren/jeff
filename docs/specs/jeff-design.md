...
8. **Auto-commit at stage boundaries; never block on a dirty tree.** Jeff owns the working tree during a run. On completing each stage it commits that stage's code + artifact + state changes as one narrow, legibly-messaged commit before entering the next stage: so every stage starts clean and produces a discrete, reviewable diff. The workflow **never halts to ask the Chef to stash or clean**; there is no dirty-tree gate, no waiver dance.
9. **Be smart about git; make judgment calls; interrupt rarely.** Routine commit / branch / squash is autonomous inside a run (this is the intended override of any default "confirm before commit"). Jeff interrupts the Chef only when genuinely necessary: surprising unrelated changes it can't attribute, an unresolvable conflict, or anything requiring force-push / history rewrite. `complexity` (`"simple" | "complex"`; absent ⇒ `"complex"`) drives the commit path: **simple** tasks (do not complect) commit directly to the trunk; **complex** tasks (complect: braid concerns, couple previously-separate things, cross subsystem boundaries, or carry non-local side effects such as a deployment) run on a short-lived **local** branch `task/<id>-<slug>` where stage-boundary commits accumulate, then squash-merge to the trunk as one clean green commit on `done` and delete the branch (full mode: no remote branch, no PR; capture any lasting lesson in project memory, never a surviving branch). Classify by complecting, not difficulty; deployment / non-local side-effects ⇒ complex; default complex when unsure; the simple-vs-complex call is made at plan. Every trunk commit must be green; backlog/capture commits land on the trunk immediately, decoupled from any code branch. Net: trunk history stays one-commit-per-task, while granular, revertable stage history exists during the task's life (and feeds kickbacks).
...
**Stage-boundary commits.** Each stage transition is a commit (Principle 8): Jeff stages the task's code, artifacts, and `task.json`/`notes.md` updates and commits them with a stage-scoped message (e.g. `task <id> · <stage>: <summary>`), then enters the next stage. This keeps the tree clean at every boundary, gives the reviewer a discrete per-stage diff, and makes kickbacks legible (each stage's effect is one commit, revertable). Jeff runs `cook validate` before each commit; CI runs it on push. Assumption: the loop starts from a clean tree and owns the working tree for the run; it commits its own work rather than ever interrupting to ask the Chef to clean up.
...
Because Jeff commits at every stage boundary (Principle 8), Jeff runs `cook validate` before each commit (CI runs it on push): structural invariants (5, 6) bind on every commit; the separation/completion invariants (1, 2, 4) bind as their fields populate, with the full done-gate (invariant 4) enforced only when `status = done`. So intermediate stage commits are never blocked for "incompleteness": only a genuine structural break or an illegitimate `done` is.
