...
10. **One bounded task-wide council research and recovery episode.** A review or audit cap is only the trigger. Council waits for all required active judgments and source-bound surviving refutes, then sends their exact blocking union through three non-identical independent inquiry packets before deterministic synthesis. The synthesis restates the problem, compares causal hypotheses and materially different strategies, and decides whether independent defects require confined repair or whether the implicated design needs reconstruction. A block may open exactly one same-task code recovery episode inside the linear stage machine; one failed recovery blocks to the operator and cannot be reset through another task.
...
Canonical code council recovery does not add a graph runtime. Its six typed selections reuse existing states: confined repair enters implement; test-contract repair enters fresh plan/test authorship and then the gate-facing review state without a production record; refactor enters the existing refactor stage; causal-subgraph reconstruction and full replan enter plan and then implement; operator escalation blocks. The original task, findings, votes, synthesis, checkpoint, and judgment history remain durable. Recovery authors, builders, and fresh judges are distinct, the normal clean gate and audit floor remain binding, and historical councils without research or recovery retain their earlier meaning.
